...
{   
    ...
    public function display_Limiter($total_Elements,$anchor)
    {
		global $min_onPage,$max_onPage;
	...
	else
	{
		...
		<META HTTP-EQUIV="Refresh" Content="0; URL=?page=1&elems=10<?php echo $anchor?>">
		<?php 
		}
		else
		{
			$selected_Page=htmlentities($_GET['page']);
			...
			if($selected_Page>1)
			{
			echo "<a title='Previous page' id='arrows'  href='?page=$previous_Page&elems=$elements_onPage$anchor'><</a>";
			}
			if($total_Pages<=7)
			{
				for($pages_Counter=1;$pages_Counter<=$total_Pages;$pages_Counter++)
				{
				echo "<a href='?page=$pages_Counter&elems=$elements_onPage$anchor'>$pages_Counter</a>" ;
				}
			}
			else
			{
				...
				echo "<a href='?page=1&elems=$elements_onPage$anchor'>1</a>";
				//if()
				...
				echo "<a href='?page=$total_Pages&elems=$elements_onPage$anchor'>$total_Pages</a>";	
			}
			echo "<span id='delmt_text'>Page $selected_Page of $total_Pages</span>";
			$next_Page=$selected_Page+1;
			if($selected_Page<$total_Pages){
			echo "<a title='Next page' id='arrows'  href='?page=$next_Page&elems=$elements_onPage$anchor'>></a>";
			}
